...
function handleEvent(event) {
  ...
  if (event.type === "join") {
    ...
    answer =
      "Thanks for inviting me... Actually, nevermind, you should be the one thanking me for looking up on your stuffs.\n\nI will answer you if you ask a question ending with ;?\n\ntry: Elon Musk;?";
  ...
  if (event.type === "message" && event.message.type === "text") {
    ...
    if (userText.slice(-2) === cmdSearch) {
      ...
      if (userText.length > 2) {
        ...
        reply(
          replyToken,
          "GOLE adala bot yang membantu kalian mencarikan sesuatu di google supaya kalian tida perlu ganti aplikasi atau buka browser biar kalian kaga males cari sendiri.\nInvit GOLE ke grup kalo butuh googling rame-rame.\n\nHow to use:\n[Search anything here][;?]\nExample:\nGoogle;?\n\nDeveloped by: \nBryan Khufa\nLinkedin\t: https://www.linkedin.com/in/bryan-khufa\nTwitter\t: https://twitter.com/BryanCoffeee"
        );
      }
...
async function getSearchHistory(userQuestion) {
  ...
  const doc = await ref
    .get()
    .catch((error) => console.error("firebase fail", error));
  if (!doc.exists) {
...
async function getGoogleSerpsbot(userQuestion) {
  ...
  try {
    const res = await got(
      `https://google-search5.p.rapidapi.com/google-serps/?q=${userQuestion}&gl=${gl}&hl=${hl}&num=5`,
      {
        headers: {
    ...
    const searchResult = result.data.results.organic.map(
...
async function getGoogleMarcelinhov(userQuestion) {
  ...
  try {
    const res = await got(
      `https://google-search1.p.rapidapi.com/google-search?q=${userQuestion}&hl=${hl}&gl=${gl}&num=5`,
      {
        headers: {
    ...
    const result = JSON.parse(res.body);
    const searchResult = result.organic.map(
      ({ title, snippet: description, url: link }) => ({
        title,
        description,
        link,
...
async function getGoogleApigeek(userQuestion) {
  ...
  try {
    const res = await got(
      `https://google-search3.p.rapidapi.com/api/v1/search/q=${userQuestion}&num=5&lr=lang_en&gl=${gl}`,
      {
        headers: {
